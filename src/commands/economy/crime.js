...
module.exports.run = async (client, message, args) => {
  ...
  if (timeout - (Date.now() - data.crimeTimeout) > 0) {
    let timeLeft = ms(timeout - (Date.now() - data.crimeTimeout));
    ...
    const crimeEmbed = new discord.MessageEmbed()
      .setColor("#0155b6")
      .setDescription(
        `You committed a crime and earned **:coin: ${amount.toLocaleString()}**`
      )
